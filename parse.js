function parse(str){
    ...
    function parse2(scope) {
        ...
        if (scope.state == "symbol") {
            ...
            if (scope.chr == "(") {
                scope.ya_exp = scope.exp
                scope.exp = []
                i--;
                scope.state = "symbol"              
                //old way                
                //i++
                //return (parse2({state:"symbol", exp:[scope.exp], val:[], chr:arr[i], level: scope.level + 1}))   
        ...
        } else if (scope.state == "possible close quote") {
            if (scope.chr == "(") {
            ...
            if (scope.chr == scope.quote_type) {
                ...
                //scope.val = []
                //scope.state = "symbol"
                scope.state = "possible close quote"
            } else if (scope.chr == "\\"){
